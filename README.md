...
- タイトルと画像をDiscordに通知
- 新着は10件ずつ1つのメッセージにまとめて送ります（Discordの仕様で1メッセージ10件まで）。1回の実行で送るのは最大50件で、それを超えた分は消さずに次回の実行に持ち越します
...
A. 増えません。1シリーズあたり最新500件までしか保存しないように
自動で古い記録を切り捨てる仕組みになっています（`src/state.py` 内の
`SEEN_LIMIT_PER_SERIES` という数値で調整できます）。

**Q. 51件以上新着があった場合、超えた分は消えますか？**
A. 消えません。今回通知した50件だけが「既読」として記録され、
